...
해당 식에는 위(Hard Margin)에는 식에는 없는 $C$ 가 존재하는데, 이는 뒤에 **error($\epsilon_i$)를 얼마나 식에 반영**할지 정하는 하이퍼 파라미터이다.
...
그리고 만약 $c$가 매우 작아지면(0) **error(epsilon)**의 영향이 매우 작아지기 때문에 모든 error를 허용할 것이라고 예상할 수 있다. 
